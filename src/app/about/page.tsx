...
export default function AboutPage(props: { data: any; query: string; variables: any }) {
  const { data } = useTina({
    query: props.query || `query {
      about(relativePath: "about.json") {
        header { isVisible useVideo bgImage title italicWord subtitle badgeText valueBoxes { label sublabel icon } }
        intro { isVisible theme badge titleLine1 titleItalic paragraphs quote quoteAuthor }
        pillars { isVisible theme list { icon title text } }
        leaders { isVisible list { name title photo quote bio } }
        philosophy { isVisible theme titleLine1 titleItalic description items { title icon desc } }
        journey { isVisible badge titleLine1 titleItalic subtitle items { title text } }
        national { isVisible badge titleLine1 titleItalic quote legend { icon text } }
        cta { isVisible title subtitle button }
      }
    ...
    data: props.data || { about: aboutData, settings: settingsData },
  });
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      {/* ── 2. Trust Banner (Merged into Intro theme or separate) ── */}

      {/* ── 3. Intro / Bridging the Gap ── */}
      {p.intro.isVisible !== false && (
        <section className={cn(
          "py-24 md:py-48 overflow-hidden relative",
          p.intro.theme === 'dark' ? "bg-[#0f172a] text-white" : p.intro.theme === 'slate' ? "bg-slate-900 text-white" : "bg-white text-[#0f172a]"
        )}>
            ...
            <div className="grid lg:grid-cols-2 gap-16 lg:gap-32 items-center">
              ...
              <div>
                <BlurFade delay={0.1}>
                  ...
                  <h2
                    className={cn(
                      "text-4xl sm:text-5xl md:text-7xl font-serif font-black mb-8 sm:mb-12 leading-[1] sm:leading-[0.85] tracking-tighter",
                      (p.intro.theme === 'dark' || p.intro.theme === 'slate') ? "text-white" : "text-[#0f172a]"
                    )}
              ...
              {/* Right: Pillars Section - Using list field */}
              {p.pillars.isVisible !== false && (
                <div className="grid grid-cols-1 gap-6">
                  {(p.pillars.list || []).map((item: any, i: number) => {
                    ...
                    return (
                      <BlurFade
                        ...
                        delay={0.2 + i * 0.15}
                        className={cn(
                          "flex gap-8 p-10 rounded-[2.5rem] border shadow-[0_20px_50px_rgba(0,0,0,0.03)] hover:shadow-2xl transition-all duration-700 group",
                          p.pillars.theme === 'dark' || p.pillars.theme === 'slate' ? "bg-white/5 border-white/10" : "bg-white border-slate-100"
                        )}
                      >
                        ...
                        <div>
                          <h3
                            className={cn(
                              "font-black font-serif text-2xl mb-4",
                              p.pillars.theme === 'dark' || p.pillars.theme === 'slate' ? "text-white" : "text-[#0f172a]"
                            )}
          ...
          <div className="container mx-auto px-6 md:px-12 relative z-10">
            <BlurFade className="text-center mb-24 md:mb-32">
              <div className="flex justify-center mb-8">
                <AnimatedGradientText>The Leadership</AnimatedGradientText>
              </div>
              <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-8xl font-serif font-black text-[#0f172a] tracking-tighter leading-[1] sm:leading-[0.85]">
                Visionary <span className="text-[#0284c7] italic font-medium">Partnership</span>
              </h2>
            </BlurFade>

            <div className="space-y-16 sm:space-y-24 md:space-y-40 max-w-6xl mx-auto">
              {(p.leaders.list || []).map((leader: any, i: number) => (
      ...
      {/* ── 5. Philosophy ── */}
      {p.philosophy.isVisible !== false && (
        <section className={cn(
          "py-24 md:py-48 relative overflow-hidden",
          p.philosophy.theme === 'dark' ? "bg-[#0f172a] text-white" : p.philosophy.theme === 'slate' ? "bg-slate-900 text-white" : "bg-white text-[#0f172a]"
        )}>
          <div className="container mx-auto px-6 md:px-12 relative z-10">
            <div className="max-w-5xl mx-auto text-center">
              <BlurFade delay={0.2} duration={0.8}>
                ...
                <h3
                  className={cn(
                    "text-4xl sm:text-5xl md:text-7xl font-serif font-black mb-8 sm:mb-10 leading-[1] sm:leading-[0.9] tracking-tighter",
                    p.philosophy.theme === 'light' ? "text-[#0f172a]" : "text-white"
                  )}
                  ...
                  {p.philosophy.items?.map((item: any, idx: number) => {
                    ...
                    return (
                      <div
                        key={idx}
                        className={cn(
                          "p-10 rounded-[2.5rem] border flex flex-col items-center text-center group hover:shadow-2xl transition-all duration-700",
                          p.philosophy.theme === 'light' ? "bg-slate-50 border-slate-100 hover:bg-white" : "bg-white/5 border-white/10 hover:bg-white/10"
                        )}
                      >
           ...
           <div className="container mx-auto px-6 md:px-12 relative z-10">
              <div className="flex flex-col lg:flex-row gap-16 lg:gap-32">
                 <div className="lg:w-1/2">
                    <BlurFade delay={0.1}>
                       <div className="mb-8">
                          <AnimatedGradientText data-tina-field={tinaField(p.journey, 'badge')}>{p.journey.badge}</AnimatedGradientText>
                       </div>
                       <h2 className="text-4xl sm:text-5xl md:text-7xl font-serif font-black text-[#0f172a] tracking-tighter leading-[0.85] mb-12">
                          <span data-tina-field={tinaField(p.journey, 'titleLine1')}>{p.journey.titleLine1}</span><br />
                          <span className="text-[#0284c7] italic font-medium" data-tina-field={tinaField(p.journey, 'titleItalic')}>{p.journey.titleItalic}</span>
                       </h2>
                       <p className="text-slate-500 text-xl font-light leading-relaxed mb-12" data-tina-field={tinaField(p.journey, 'subtitle')}>
                          {p.journey.subtitle}
                       </p>
                    </BlurFade>
                 </div>
                 <div className="lg:w-1/2 space-y-8">
                    {p.journey.items?.map((item: any, i: number) => (
                       <BlurFade key={i} delay={0.2 + i * 0.1} className="p-8 bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl transition-all duration-500">
                          <h4 className="font-black text-[#0f172a] text-xl mb-4" data-tina-field={tinaField(item, 'title')}>{item.title}</h4>
                          <p className="text-slate-500 font-light leading-relaxed" data-tina-field={tinaField(item, 'text')}>{item.text}</p>
                       </BlurFade>
                    ))}
                 </div>
              </div>
              ...
              <div className="flex flex-col items-center mb-16">
                ...
                >
                  ...
                  <span
                    ...
                    data-tina-field={p.national.titleItalic ? tinaField(p.national, 'titleItalic') : undefined}
                  >
