        ...
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          <div className="grid lg:grid-cols-12 gap-12 lg:gap-20 items-stretch">
             ...
             <div className="lg:col-span-7 flex flex-col justify-center gap-8">
                ...
                >
                   <h3 data-tina-field={tinaField(p.strategy, 'title')} className="text-3xl lg:text-4xl font-serif font-black text-[#0f172a] mb-6 tracking-tighter leading-tight flex items-center gap-4 group-hover:text-[#0284c7] transition-colors">
                     <TrendingUp className="text-[#0284c7]" size={36} />
                     {p.strategy.title}
                   </h3>
                   <p data-tina-field={tinaField(p.strategy, 'description')} className="text-lg md:text-xl text-slate-500 leading-relaxed font-light mb-6">
                     {p.strategy.description}
                   </p>
                   <p data-tina-field={tinaField(p.strategy, 'subtext')} className="text-sm md:text-base text-slate-400 leading-relaxed font-normal">
                     {p.strategy.subtext}
                   </p>
                </BlurFade>
                ...
                >
                   ...
                   <div className="relative z-10">
                     <div data-tina-field={tinaField(p.partner, 'badge')} className="inline-flex px-3 py-1 bg-[#0284c7]/10 text-[#0284c7] text-[10px] uppercase font-black tracking-widest rounded-full mb-6">{p.partner.badge}</div>
                     <h4 data-tina-field={tinaField(p.partner, 'title')} className="text-2xl md:text-3xl font-serif font-black text-[#0f172a] mb-4 tracking-tight leading-tight group-hover:text-[#0f172a] transition-colors">{p.partner.title}</h4>
                     <p data-tina-field={tinaField(p.partner, 'description')} className="text-slate-500 leading-relaxed text-base md:text-lg font-light">
              ...
              {p.reach.items.map((item: any, i: number) => {
                ...
                return (
                  <div key={i} className="p-12 rounded-[3.5rem] bg-slate-50 border border-slate-100 hover:scale-[1.05] transition-all duration-500">
                    ...
                    <h4 data-tina-field={tinaField(item, 'title')} className="text-2xl font-serif font-black text-[#0f172a] mb-6 tracking-tight">{item.title}</h4>
                    <p data-tina-field={tinaField(item, 'desc')} className="text-slate-500 text-sm font-light leading-relaxed">{item.desc}</p>
                  </div>
                );
              })}
           </div>
