      ...
      <section className="py-16 md:py-24 bg-slate-50 relative overflow-hidden">
        ...
        <div className="container mx-auto px-5 sm:px-6 md:px-12 relative z-10">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12 md:mb-16">
            <BlurFade delay={0.1}>
              <div className="mb-5" data-tina-field={tinaField(p?.clinicalExcellence, 'badge')}>
          ...
          <BlurFade delay={0.2} className="bg-[#0f172a] rounded-3xl md:rounded-[2.5rem] p-8 md:p-14 overflow-hidden relative border border-white/5">
            ...
            <div className="relative z-10">
              <div className="mb-12">
                 <div data-tina-field={tinaField(p?.process, 'badge')} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-[#38bdf8] text-[9px] font-black uppercase tracking-[0.4em] mb-6">
                    {p?.process?.badge || ''}
                 </div>
                 <h3 className="text-4xl md:text-5xl font-serif font-black text-white tracking-tighter mb-4">
                    <span data-tina-field={tinaField(p?.process, 'title')}>{p?.process?.title || ''}</span> <span data-tina-field={tinaField(p?.process, 'titleItalic')} className="text-[#0284c7] italic">{p?.process?.titleItalic || ''}</span>
                 </h3>
                 <p data-tina-field={tinaField(p?.process, 'description')} className="text-white/40 text-lg font-light max-w-2xl leading-relaxed">
                ...
                {p?.process?.steps?.map((step: any, i: number) => {
                  ...
                  return (
                    <BlurFade delay={0.3 + i * 0.12} key={i} className="relative group">
                      <div data-tina-field={tinaField(step)}>
        ...
        <div className="container mx-auto px-5 sm:px-6 md:px-12 relative z-10">
          <div className="max-w-5xl mx-auto text-center">
            <BlurFade delay={0.2} duration={0.8}>
              ...
              <h3 data-tina-field={tinaField(p?.bottomCta, 'quote')} className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-serif text-white mb-10 md:mb-16 leading-[0.9] tracking-tighter -mt-6">
                {p?.bottomCta?.quote?.includes(' in-house') ? p.bottomCta.quote.split(' in-house')[0] : (p?.bottomCta?.quote || '')}
                {p?.bottomCta?.quote?.includes(' in-house') && <span className="text-[#0284c7] italic"> in-house leadership</span>}
                <br />{p?.bottomCta?.quote?.includes('leadership ') ? p.bottomCta.quote.split('leadership ')[1] : ''}
              </h3>
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center items-center">
                <Link href="/contact">
                  <ShimmerButton background="#0284c7" shimmerColor="rgba(255,255,255,0.6)" borderRadius="9999px" className="group px-12 py-6">
                    <span data-tina-field={tinaField(p?.bottomCta, 'primaryCta')} className="text-[11px] font-black uppercase tracking-[0.3em] text-white">{p?.bottomCta?.primaryCta || 'Contact'}</span>
                ...
                >
                  <Phone size={16} /> <span data-tina-field={tinaField(p?.bottomCta, 'phone')}>{p?.bottomCta?.phone || ''}</span>
                </a>
              </div>
            </BlurFade>
          </div>
