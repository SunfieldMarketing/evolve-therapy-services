...
export default function ServicesPage(props: { data: any, query: string, variables: any }) {
  const { data } = useTina({
    query: props.query || `query { 
      services(relativePath: "services.json") { 
        hero { isVisible badge titleLine1 titleItalic description heroValues { icon title subtitle } } 
        trust { icon text } 
        showcase { isVisible badge titleLine1 titleItalic services { title desc details icon image slug alignment } } 
        methodology { isVisible theme badge titleLine1 titleItalic items { title icon desc } sidebarTitle sidebarItalic sidebarQuote sidebarIcon } 
        advantage { isVisible theme badge titleLine1 titleItalic items { title icon desc } } 
        pricing { isVisible theme badge title titleItalic description tiers { level title desc features featured } }
      }
    ...
    data: props.data || { services: servicesData, settings: settingsData },
  });
  ...
  return (
    <main className="min-h-screen bg-white">
            ...
            <div className="w-full">
              <BlurFade delay={0.2}>
                 ...
                 <div className="flex flex-col lg:flex-row items-start lg:items-center gap-12 lg:gap-24">
                    <Link href="/contact" className="inline-flex group shrink-0 relative z-50 pointer-events-auto">
                ...
                <BlurFade delay={0.1} key={i} className="group">
                  <div className={cn(
                    ...
                    <div className="w-full lg:w-1/2">
                      ...
                      <Link href={`/services/${service.slug}`} className="relative z-50 pointer-events-auto">
      ...
      {/* ── Methodology ── */}
      {p.methodology.isVisible !== false && (
        <section className={cn(
          "py-32 md:py-56 overflow-hidden relative",
          p.methodology.theme === 'dark' ? "bg-[#0f172a] text-white" : p.methodology.theme === 'slate' ? "bg-slate-900 text-white" : "bg-slate-50 text-[#0f172a]"
        )}>
           <div className="container mx-auto px-6 md:px-12">
              <div className="max-w-4xl mb-32">
                 <BlurFade delay={0.1}>
                    ...
                    <h2 className={cn(
                      "text-6xl md:text-8xl font-serif font-black tracking-tighter leading-[0.85] mb-12",
                      (p.methodology.theme === 'dark' || p.methodology.theme === 'slate') ? "text-white" : "text-[#0f172a]"
                    )}>
                    ...
                    {(p.methodology.items || []).map((item: any, i: number) => (
                      <BlurFade key={i} delay={0.2 + i * 0.1}>
                         <div className="flex gap-10 group" data-tina-field={tinaField(item, 'title')}>
                            ...
                            <div>
                               <h4 className={cn(
                                 "text-3xl font-serif font-black mb-4",
                                 (p.methodology.theme === 'dark' || p.methodology.theme === 'slate') ? "text-white" : "text-[#0f172a]"
                               )}>{item.title}</h4>
                 ...
                 <BlurFade delay={0.5} className="h-full">
                   <div className="bg-[#0f172a] rounded-[5rem] p-16 md:p-24 text-white relative overflow-hidden group h-full flex flex-col justify-between shadow-[0_50px_100px_rgba(2,132,199,0.2)]">
                      ...
                      <div className="relative z-10 flex flex-col h-full justify-between">
                         ...
                         <Link href="/contact" className="group/btn inline-flex items-center gap-4 text-white font-black uppercase text-[11px] tracking-[0.4em] hover:text-[#38bdf8] transition-colors mt-auto relative z-50 pointer-events-auto">
      ...
      {p.pricing.isVisible !== false && (
        <Pricing data={p.pricing} parentField="pricing" />
      )}

      {p.advantage.isVisible !== false && (
        <section className={cn(
          "py-32 md:py-56 overflow-hidden relative",
          p.advantage.theme === 'dark' ? "bg-[#0f172a] text-white" : p.advantage.theme === 'slate' ? "bg-slate-900 text-white" : "bg-white text-[#0f172a]"
        )}>
           <div className="container mx-auto px-6 md:px-12 relative z-10">
              <div className="text-center max-w-4xl mx-auto mb-32">
                 <BlurFade delay={0.1}>
                    ...
                    <h2 className={cn(
                      "text-6xl md:text-9xl font-serif font-black tracking-tighter leading-[0.82]",
                      (p.advantage.theme === 'dark' || p.advantage.theme === 'slate') ? "text-white" : "text-[#0f172a]"
                    )}>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-7xl mx-auto">
                 {(p.advantage.items || []).map((item: any, i: number) => (
                   <BlurFade key={i} delay={0.2 + i * 0.1}>
                      <div className={cn(
                        "group p-12 rounded-[3rem] border transition-all duration-700 h-full flex flex-col justify-between relative overflow-hidden",
                        p.advantage.theme === 'light' ? "bg-slate-50 border-slate-100 hover:bg-white hover:border-[#0284c7]/30" : "bg-white/5 border-white/10 hover:bg-white/10 hover:border-[#0284c7]/50"
                         ...
                         <div className="relative z-10">
                            ...
                            <h4 className={cn(
                              "text-2xl font-serif font-black mb-8 tracking-tight group-hover:text-[#0284c7] transition-colors",
                              (p.advantage.theme === 'dark' || p.advantage.theme === 'slate') ? "text-white" : "text-[#0f172a]"
                            )}>{item.title}</h4>
