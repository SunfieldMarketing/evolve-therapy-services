...
export default function ChatBot() {
  ...
  const getThreadedResponse = (query: string) => {
    ...
    if (!knowledge) return { text: "I am analyzing the Evolve clinical database. Please wait one moment..." };
    ...
    if (q.includes('state') || q.includes('location') || q.includes('where') || q.includes('operate') || (q.includes('which') && wasDiscussingStates)) {
        ...
        if (stateMatch) {
            return {
                text: `Yes, we maintain full operational oversight in ${stateMatch}. Across our entire 17-state footprint, we ensure 100% clinical compliance and revenue retention. Would you like to view our regional directors' coverage map?`,
        ...
        return {
            text: `${bridge}Evolve currently provides regional management in the following ${states.length} states: ${states.join(', ')}. In every region we serve, we focus on transitioning facilities to high-performing, in-house models. Shall we discuss how we can support your specific region?`,
    ...
    if (q.includes('grow') || q.includes('result') || q.includes('ebitda') || q.includes('improve') || (q.includes('help') && wasDiscussingPerformance)) {
        ...
        return {
            text: `${bridge}${fact} This ensures that your facility retains 100% of the therapy revenue. Shall we review your current Medicaid case mix together?`,
    ...
    if (q.includes('service') || q.includes('do you do') || q.includes('what can you') || (q.includes('help') && !wasDiscussingPerformance)) {
        ...
        return {
            text: `${bridge}we specialize in ${s.join(', ')}. Our core strength is the In-House Transition, where we handle recruitment and clinical education so that you maintain absolute control. Shall we discuss which of these services would most positively impact your operations today?`,
    ...
    if (bestMatch) {
        ...
        return {
            text: `${bridge}regarding ${title}: Our model focuses on ${bestMatch.hero?.subtext || 'operational excellence'}. This ensures that you eliminate hidden management fees while maintaining 100% control of your department. Would you like a detailed analysis of this workflow?`,
    ...
    // FINAL FALLBACK
    return {
        text: "That is a vital operational inquiry. Based on our conversation so far, I would like to connect you with our leadership team for a brief strategy analysis. This will allow us to provide a pinpoint-accurate answer tailored to your specific census. Would that be helpful?",
        cta: { text: "Connect with Leadership", link: "/contact" }
    };
  };
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-20 right-0 w-[90vw] max-w-[350px] sm:max-w-none sm:w-[420px] h-[620px] bg-white rounded-[2.5rem] shadow-[0_30px_90px_-15px_rgba(15,23,42,0.15)] border border-slate-100 flex flex-col overflow-hidden"
          >
            {/* Header */}
